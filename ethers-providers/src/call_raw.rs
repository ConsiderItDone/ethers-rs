...
impl<'a, P: JsonRpcClient> CallBuilder<'a, P> {
    pub fn poll(mut self) -> Result<Bytes, ProviderError> {
        loop {
            match self {
                CallBuilder::Build(ref call) => return call.execute(),
            }
        }
    }
}
...
impl<'a, P: JsonRpcClient> Caller<'a, P> {
    ...
    fn execute(&self) -> Result<Bytes, ProviderError> {
...
mod tests {
    ...
    fn test_state_overrides() {
        ...
        provider.call_raw(&tx).state(&state).unwrap();
        ...
        let bytes = provider.call_raw(&tx).state(&state).poll().unwrap();
        assert_eq!(U256::from_big_endian(&bytes), balance);
        ...
        let bytes = provider.call_raw(&tx).state(&state).poll().unwrap();
        let deployed = Address::from_slice(&bytes[12..]);
        ...
        let bytes = provider.call_raw(&tx).state(&state).poll().unwrap();
        assert_eq!(H256::from_slice(&bytes), val);
    }
}
